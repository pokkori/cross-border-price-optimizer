...
*   **外部プラットフォーム（国内）**: メルカリ（10%）、ヤフオク（8.8%）、ラクマ（6.6%）、PayPayフリマ（5%）。
*   **外部プラットフォーム（海外）**: eBay（12.9%）、Amazon（15%）、StockX（10%）、Mercari US（10%）。
...
*   **カラム**:
    ...
    *   `base_fee_percentage` (DECIMAL(5,4), NOT NULL): 基本手数料率。
...
### 4.2 計算式
`利益 = (海外販売価格 * 為替) - (国内仕入価格 + 国際送料 + 関税 + プラットフォーム手数料)`
...
*   **エラーメッセージ**: `translateErrorMessage` で送料・関税・国内仕入価格などの日本語メッセージに変換。Supabase 未設定時は「設定後は、ターミナルで Ctrl+C でサーバーを停止し、再度 npm run dev を実行してください。」を追加表示。
...
## 11. 今後の拡張 (Future Considerations / Expansion Points)
